...
## 影子模型 (Shadow Model, Mshadow)

原理：知识蒸馏组件，基于 Transformer 编码器 [13] 学习流量序列模式，提供“影子”知识（非直接分类攻击，而是预测掩码字段 [44]），生成丰富神经元激活用于 FP 分析。脑科学类比：模拟大脑神经元状态，异常激活表示欠拟合（未见流量）。
输入：Tokenized 流量 XT（序列：前 LMax 包的 3 特征 - 到达间隔、协议号、包长；掩码端口 yp）。
输出：激活模式 O（最后全连接层输出，维度高，提供神经元状态）。
训练流程（TokenConstruction + ShadowModelTrain）：
Token 构建：组织流为序列，掩码端口（预测目标），K-Means 聚类 yp → yT（簇标签，e.g., 服务类型组）。
公式：yi_T = arg min_j ||C_j - yi_p||^2（C 为 K 簇中心）。

Transformer 训练：堆叠双向编码器（多头注意力），预测 yT。
嵌入：FC0(输入) + 位置编码 e。
注意力：Q/K/V = W * 输入；注意力 = Softmax(QK^T / √d_k) * V；输出 = ReLU(注意力)。
损失：Li = (1/LMax) ||Softmax(O_NL^i) - OneHot(yi_T)||^2_2（MSE）。
优化：Adam，提取 O = [O1, ..., ON1]（所有流激活）。


测试流程：对警报 a Tokenize → 运行 Mshadow → 获 O_a（仅统计特征，避免泄露）。
机理细节：
序列学习：Transformer 捕获包间依赖（e.g., P2P 共识模式），端口预测提供多样标签（零成本）。
...
输出：FP/TP 分类（距离阈值判断）。
训练流程（ActivationModelTrain）：
平均/归一化：A = MinMax({a1, ..., aN1})。
DBSCAN 聚类：Ca = DBSCAN(A; ε, minPoint)（ε: 邻域半径，minPoint: 最小簇点）。
核心：密度-based，自动簇数，无需预设 K。


测试流程：对 O_a 计算 dist(O_a, Ca)（e.g., 欧氏距离到最近簇中心）；dist > θ → FP（异常）；否则 → TP。
机理细节：
异常检测：DBSCAN 标记噪声点为异常（未见模式）；θ 基于验证集调优。
无监督：仅用训练激活学习“正常”，零标注成本。
交互：后接 MD 警报 + Mshadow 输出；FP 可用于终身学习微调 [15],[16]。
